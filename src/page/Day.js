...
const Day = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const response = await fetch("precious-exploration-production.up.railway.app/day", {
                    method: "GET",
                    headers: {
    ...
    const handleSubmit = (event) => {
        ...
        const DoCreate = async () => {
            const response = await fetch("precious-exploration-production.up.railway.app/createday",
                {
                    method: "POST",
                    headers: {
                        Accept: "application/json",
    ...
    const ToDelete = (index) => {
        ...
        if (isConfirmed) {
            ...
            const Delete = async () => {
                const response = await fetch(`precious-exploration-production.up.railway.app/deleteday/${index}`, {
                    method: "DELETE",
                    headers: {
                        Accept: "application/json",
                        "content-type": "application/json"
                        ...
                        <tbody>
                            {totalday.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <th className='col-2 border text-center' scope='col'><Link to={`/home/app/${item.Id}`} ><button className='btn border bg-success'>แก้ไข</button></Link></th>
                                    <th className='col-2 border text-center' scope='col'><Link to={`/home/eveing/${item.Id}`}><button className='btn border bg-success'>แก้ไข</button></Link></th>
